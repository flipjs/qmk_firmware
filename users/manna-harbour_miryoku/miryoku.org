...
[[https://github.com/manna-harbour/miryoku/blob/master/README.org][Additional visualisations]] are provided outside QMK.
...
e.g. ~MIRYOKU_BASE=QWERTY~ to the make command line when building.
...
for each hand and so an alternative mapping is provided.  The right half as is
as follows: The rightmost column bottom 3 keys is the pinkie column.  The middle
4 columns top 3 rows are for the remaining fingers.  The pinkie column is one
row lower than the other columns to provide some column stagger.  The bottom row
left 3 keys are the thumb keys.  The remaining keys are unused.  To select this
mapping, append ~MIRYOKU_LAYOUT=SPLIT~ to the make command line when building.
...
make keebio/levinson:manna-harbour_miryoku:flash MIRYOKU_LAYOUT=SPLIT # levinson
#+END_SRC
...
#if defined MIRYOKU_LAYOUT_SPLIT
#define LAYOUT_miryoku(\
K00,   K01,   K02,   K03,   K04,   K05,   K06,   K07,   K08,   K09,\
K10,   K11,   K12,   K13,   K14,   K15,   K16,   K17,   K18,   K19,\
K20,   K21,   K22,   K23,   K24,   K25,   K26,   K27,   K28,   K29,\
...
To request support for additional base layer alphas, layouts, or keyboards, or
for other issues, please [[https://github.com/manna-harbour/qmk_firmware/issues/new][open an issue]].

For more general discussion, please join a [[https://www.reddit.com/search/?q=author%3Amanna_harbour%20miryoku&sort=new][recent thread]] or [[https://www.reddit.com/message/compose/?to=manna_harbour][send a PM]].
